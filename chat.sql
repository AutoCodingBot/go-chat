...
CREATE TABLE `group_messages` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `group_id` int DEFAULT NULL COMMENT '群id,自增,非uuid',
  `from_user_id` int DEFAULT NULL COMMENT '发送人ID',
  `content` varchar(250) DEFAULT NULL COMMENT '消息内容',
  `url` varchar(350) DEFAULT NULL COMMENT '''文件或者图片地址''',
  `pic` text COMMENT '缩略图',
  `content_type` smallint DEFAULT NULL COMMENT '''消息内容类型：1文字，2语音，3图片''',
  `created_at` datetime(2) DEFAULT NULL COMMENT '创建时间',
  `updated_at` datetime(2) DEFAULT NULL COMMENT '更新时间',
  `deleted_at` bigint unsigned DEFAULT NULL COMMENT '删除时间戳',
  PRIMARY KEY (`id`),
  KEY `idx_messages_deleted_at` (`deleted_at`),
  KEY `idx_messages_from_user_id` (`from_user_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='群消息表';